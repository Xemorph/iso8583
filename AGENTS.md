...
- `...Cached` variants: process-wide cache per absolute path. `CacheValidation::CheckEveryCall` (default, stats the file each call, ~1 µs) vs `CacheValidation::TrustUntilInvalidated` (~25 ns hit, no change detection — call `SpecDecoder::invalidateCache(path)` manually). `SpecDecoder::clearCache()` empties everything.
...
1. **`.smap` sidecar is written next to the spec file** (`abs_path + ".smap"`, JSON; format in `src/_sourcemap.hh`, written in `src/_preprocessor.cc`). Consequences:
   - The **directory containing the spec must be writable**. In read-only deployments (common in bank environments) loading still succeeds, but the library logs `WARN "[SourceMap] Konnte Sidecar nicht schreiben"` on **every** load.
